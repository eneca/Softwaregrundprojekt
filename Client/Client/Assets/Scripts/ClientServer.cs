...
public class ClientServer : MonoBehaviour
{
    ...
    private async void ConnectToS()
    {
        ...
        {
            Debug.Log(e);
            _menu.GetComponent<Menu>().ShowDisconnectError();
            _menu.GetComponent<Menu>().StopLoading();
        }
    }
    ...
    {
        string s = JsonConvert.SerializeObject(   //Weils kein JsonObject gibt
         new
         {
    ...
    private void DecryptMessage(string rcvmsg)
    {
        ...
        Debug.LogWarning(json.payloadType + " " + json.payload);
        switch (json.payloadType)
        {
            case "joinResponse": //unicast

                JoinResponseJson jr = JsonConvert.DeserializeObject<JoinResponseJson>(json.payload.ToString());
                s = jr.message;

                SendToS(CreateContainer("teamConfig", _menu.GetComponent<Menu>().sendToServer));//sends Json to server
            ...
            case "snapshot": //broadcast
                ...
                _visual.GetComponent<Visualize>().VisualizeSnapshot(sj);
            ...
            case "pauseResponse": //broadcast
                ...
                _gLoop.GetComponent<GameLoop>().Pause(b);

                if (b)
                {
                    _menu.GetComponent<Menu>().Pause();
                }
                else
                {
                    _menu.GetComponent<Menu>().Unpause();
                }
            ...
            case "matchFinish": //broadcast
                _gLoop.GetComponent<GameLoop>().Pause(true);
                ...
                _menu.GetComponent<Menu>().WinOrDefeat(lp, rp, s2, s);
                showDisc = false;
                Disconnect();
                break;
            ...
            case "reconnect": //unicast
                ...
                _visual.GetComponent<Visualize>().VisualizeSnapshot(rsj);
    ...
    private void DissolveMatchstart(string matchString)
    {
        ...
        if (s == username.text)
        {
            _visual.GetComponent<Visualize>().DissolveTeamConfig(tcl, tcr, true);
        }
        else
        {
            _visual.GetComponent<Visualize>().DissolveTeamConfig(tcl, tcr, false);
        }

        _menu.GetComponent<Menu>().StopLoading();//Stops displaying loadingscreen
    }
    ...
    private void DissolveNext(string next)
    {
        ...
        Debug.Log("Dissolve Next:" + nj.turn);
        _gLoop.GetComponent<GameLoop>().ResolveNext(nj.timeout, nj.type, nj.turn);
    }
